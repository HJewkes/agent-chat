...
import { AnalyticsAccumulator } from '../agents/analytics/accumulator.js'
...
function makeAssistantEvent(
  content: unknown[],
  opts: { timestamp?: string; model?: string; usage?: Record<string, number>; isSidechain?: boolean } = {},
) {
  return {
    type: 'assistant',
    sessionId: 'sess-1',
...
describe('AnalyticsAccumulator', () => {
  ...
  it('counts single user + assistant turn', () => {
    ...
    acc.ingest(
      makeAssistantEvent([{ type: 'text', text: 'Hi there!' }], { timestamp: '2026-01-01T00:00:05Z' }),
    )
  ...
  it('pairs tool_use and tool_result into ToolCall', () => {
    ...
    acc.ingest(
      makeAssistantEvent([{ type: 'tool_use', id: 'tu_1', name: 'Read', input: { file_path: '/a.ts' } }], {
  ...
  it('records error tool results', () => {
    acc.ingest(
      makeAssistantEvent([{ type: 'tool_use', id: 'tu_1', name: 'Bash', input: { command: 'npm test' } }], {
  ...
  it('emits pending for orphaned tool_use', () => {
    acc.ingest(makeAssistantEvent([{ type: 'tool_use', id: 'tu_orphan', name: 'Bash', input: {} }]))
  ...
  it('detects compaction', () => {
    acc.ingest(
      makeUserEvent('This session is being continued from a previous conversation that ran out of context.', {
        timestamp: '2026-01-01T00:00:00Z',
      }),
    )
  ...
  it('counts sidechain events', () => {
    acc.ingest(makeAssistantEvent([{ type: 'text', text: 'subagent work' }], { isSidechain: true }))
    acc.ingest(makeAssistantEvent([{ type: 'text', text: 'more subagent' }], { isSidechain: true }))
  ...
  it('skips progress and file-history-snapshot events', () => {
    ...
    acc.ingest({ type: 'file-history-snapshot', sessionId: 'sess-1', timestamp: '2026-01-01T00:00:01Z' })
