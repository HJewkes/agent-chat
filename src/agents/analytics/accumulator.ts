...
export function extractCompactionSummary(content: string): string | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('Here is a summary') || line.includes('ran out of context')) continue
    startIdx = i
    break
  }
...
/** Walks a Claude Code transcript's parsed JSONL rows, one `ingest()` per row, and folds them into `SessionAnalytics`. */
export class AnalyticsAccumulator {
  ...
  ingest(raw: unknown, byteOffset?: number): void {
    ...
    if (!this.sessionId && event.sessionId) this.sessionId = event.sessionId as string
    if (!this.projectDir && event.cwd) this.projectDir = event.cwd as string
    if (!this.gitBranch && event.gitBranch) this.gitBranch = event.gitBranch as string
    if (!this.claudeVersion && event.version) this.claudeVersion = event.version as string
    if (!this.slug && event.slug) this.slug = event.slug as string
  ...
  private handleAssistant(event: Record<string, unknown>, timestamp: string, byteOffset?: number): void {
    ...
    if (usage) {
      ...
      if (!this.serviceTier && usage.service_tier) this.serviceTier = usage.service_tier as string
    ...
    if (assistantText) this.assistantTexts.push(assistantText.slice(0, 2000))
  ...
  private trackToolInput(toolName: string, input: Record<string, unknown>): void {
    ...
    if (toolName === 'Task' || toolName === 'TaskCreate') this.subagentCount++
  ...
  private handleUser(event: Record<string, unknown>, timestamp: string, byteOffset?: number): void {
    ...
    if (event.planContent && !this.planPresent) this.planPresent = true
    if (event.permissionMode && !this.permissionMode) this.permissionMode = event.permissionMode as string
  ...
  private extractTaskRefs(text: string): void {
    ...
    while ((match = TASK_REF_PATTERN.exec(text)) !== null) {
      if (match[1]) this.taskRefs.add(match[1])
    }
  }
  ...
  finalize(): SessionAnalytics {
    ...
    for (const [path, ops] of this.filesTouched) {
      if (ops.has('Write') || ops.has('Edit')) filesWritten.push(path)
    }
