...
function classifyType(
  ...
  const hasTestBash = bashCalls.some(tc => BASH_TEST_PATTERN.test((tc.input.command as string) ?? ''))
...
function classifyOutcome(analytics: SessionAnalytics): 'success' | 'partial' | 'abandoned' | 'unknown' {
  const total = analytics.toolCalls.length

  if (total === 0 && analytics.userTurns < 2) return 'unknown'
  if (total < 5 || analytics.errorRate > 0.3) return 'abandoned'

  const tasksCompleted = analytics.taskRefs.length // approximation; no delivery-layer link here
