...
/** Detects friction signals — repeated tool calls, error loops, user corrections — while walking a transcript. */
export class FrictionDetector {
  ...
  onHookPrevention(timestamp: string, detail: string): void {
    this.signals.push({ kind: 'hook_prevention', timestamp, detail })
  }
