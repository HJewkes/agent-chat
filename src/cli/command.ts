...
export const Report = z.object({ ok: z.boolean(), lines: z.array(z.string()) })
...
/** Adds a registry verb under `parent`, spelled and described exactly as its definition says. */
export function addVerb<Args>(parent: Commander, verb: Verb<Args>): void {
  const sub = parent.command(commandPath(verb.name).at(-1) ?? verb.name).description(verb.description)
