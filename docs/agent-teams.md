...
| Stage       | Who                       | What                                                                                                                                                                                                                              |
| ----------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **create**  | the supervisor, at spawn  | `worktreeStrategy.allocate` cuts `agent-chat/<name>` at `.worktrees/<name>`, under a per-machine budget (3 by default; `AGENT_CHAT_WORKTREE_BUDGET=<n>` in the broker's environment raises it, CC-98). The allocation is recorded in `runtime.json`, which is what makes every later stage possible after a broker restart. |
