...
Status as of 2026-09-18: slice 1 is on branch `feat/CC-106-registry-slice-1`. It converts `chat_list`,
`chat_send` and `agent retire`. The other 22 tools and about 35 CLI verbs still use the hand-written
path, and both paths run side by side.
...
| `chat_activity`    | `src/server/tools.ts:347`          | `name*`, `limit` (number, 15 default, 50 cap)                                                                                          | none                                          | broker `activity`                                                    |
...
| `chat_inbox`       | `src/server/tools.ts:430`          | `limit` (number, 10 default, 50 cap)                                                                                                   | `inbox` is the human queue, different         | broker `inbox`                                                       |
...
| `agent_surface`    | `src/server/tools.ts:625`          | `name*`                                                                                                                                | `agent surface`: same frame, different text   | broker `surface`                                                     |
| `agent_background` | `src/server/tools.ts:644`          | none                                                                                                                                   | none                                          | broker `background`, registration guard                              |
| `agent_profiles`   | `src/server/tools.ts:655`          | none                                                                                                                                   | `profiles`: different layout                  | profile files on disk, no broker                                     |
| `agent_list`       | `src/server/tools.ts:663`          | none                                                                                                                                   | `agent ls`: different roster, relay parses it | broker `agents`, budget cache                                        |
| `agent_logs`       | `src/server/tools.ts:670`          | `name*`, `limit` (number, 10 default, 20 cap)                                                                                          | none                                          | broker `agents`, transcript on disk                                  |
| `chat_transcript`  | `src/server/tools.ts:692`          | `name`, `limit` (number, 12 default, 30 cap)                                                                                           | none                                          | own session id from env, or broker `agents`; transcript on disk      |
| `session_budget`   | `src/server/tools.ts:719`          | `name`                                                                                                                                 | `agent budget`: lists all, different          | own session id from env, or broker `agents`; budget cache            |
